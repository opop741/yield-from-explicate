...
# 1、类middle_gen加了while后可以把子生成器返回的StopIteration异常及return返回的值或None“劫持”，不会抛出至调用方处，也不会终止委托方生成器(应该是yield from语句已经处理了)
# 2、子生成器抛出StopIteration异常时，进入委托生成器的只有子生成器的return值或None，其StopIteration异常不会进入委托生成器中(起码从效果上是这样的)
